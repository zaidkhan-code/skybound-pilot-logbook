    ...
    >
      <div className="h-16 px-4 flex items-center border-b border-sidebar-border">
        <Link to="/dashboard" className="flex items-center gap-2">
          <div className="bg-sidebar-primary p-1.5 rounded">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-sidebar-primary-foreground">
          ...
          <span className={`text-lg font-bold transition-all duration-300 ${!open && "md:hidden"}`}>SkyBound</span>
        </Link>
      </div>
          ...
          {navItems.map((item) => (
            <li key={item.path}>
              ...
              >
                ...
                <span className={`transition-opacity duration-300 ${!open && "md:hidden"}`}>{item.title}</span>
              </NavLink>
            </li>
          ))}
        </ul>
      ...
      <div className="p-4 border-t border-sidebar-border">
        ...
        >
          ...
          <span className={`transition-opacity duration-300 ${!open && "md:hidden"}`}>Help & Support</span>
        </NavLink>
      </div>
    </aside>
  );
